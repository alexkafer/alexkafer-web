...
import StatusHudChrome from "@/components/chrome/status-hud-chrome";
...
export default function StatusPage() {
  ...
  const lastDeployRelative = lastCommit ? formatRelativeTime(lastCommit.date) : "—";
  const buildRelative = formatRelativeTime(buildInfo.buildTime);
  const daysSinceFirst = buildInfo.firstCommitDate
    ? Math.floor((Date.now() - new Date(buildInfo.firstCommitDate).getTime()) / 86_400_000)
    : 0;
    ...
    <>
      ...
      >
        ...
        <header className="border border-cyan/20 bg-void-800/40 backdrop-blur-sm">
          ...
          <div className="grid gap-4 px-4 py-6 sm:grid-cols-[auto_1fr] sm:items-center">
            ...
            <p className="text-pretty text-sm leading-relaxed text-mute-300">
              Static-rendered Next.js 14 app deployed to Cloudflare&apos;s edge via OpenNext. Most
              pages serve from cache in under 50&nbsp;ms globally. The A/B experiment in{" "}
              <Link href="/#lab" className="text-cyan hover:underline">
                {"// 04 LAB"}
              </Link>{" "}
              writes to a Cloudflare D1 SQLite database living in the same data centers.
            </p>
          </div>
        </header>

        {/* TELEMETRY ROW */}
        <section aria-label="Telemetry" className="mt-6 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
          ...
          />
          <Stat label="LAST DEPLOY" value={lastDeployRelative} hint={lastCommit?.sha ?? "—"} />
          <Stat
            label="BUILD"
        ...
        <section aria-labelledby="infra-h" className="mt-10 border border-cyan/20 bg-void-800/30">
          ...
          <div className="grid gap-px bg-cyan/10 sm:grid-cols-2 lg:grid-cols-3">
            {STACK.map((s) => (
              <div key={s.label} className="bg-void-800/60 p-4">
                <p className="text-[10px] uppercase tracking-widest text-cyan/60">{s.label}</p>
                <p className="mt-1 text-sm font-semibold text-mute-100">{s.value}</p>
                <p className="mt-2 text-xs leading-relaxed text-mute-500">{s.detail}</p>
              </div>
            ))}
          </div>
        </section>
        ...
        {/* DEPLOYMENT HEATMAP */}
        <section aria-labelledby="heatmap-h" className="mt-10 border border-cyan/20 bg-void-800/30">
          <PanelHeader id="heatmap-h" code="02" title="DEPLOYMENT HEATMAP · 90D" />
        ...
        {/* BUILD METADATA */}
        <section aria-labelledby="build-h" className="mt-10 border border-cyan/20 bg-void-800/30">
          <PanelHeader id="build-h" code="03" title="BUILD METADATA" />
          <dl className="grid gap-px bg-cyan/10 sm:grid-cols-2">
            ...
            <Field
              term="BUILD TIME"
              value={<time dateTime={buildInfo.buildTime}>{buildInfo.buildTime}</time>}
            />
...
function Stat({ label, value, hint }: { label: string; value: string; hint: string }) {
  return (
    <div className="border border-cyan/15 bg-void-800/40 p-4">
      <p className="text-[10px] uppercase tracking-widest text-cyan/60">{label}</p>
      <p className="mt-1 text-xl font-semibold text-mute-100">{value}</p>
      <p className="mt-1 break-all text-[10px] uppercase tracking-wider text-mute-500">{hint}</p>
    </div>
  );
}

function PanelHeader({ id, code, title }: { id: string; code: string; title: string }) {
  return (
    <h2
      id={id}
...
function Field({ term, value }: { term: string; value: React.ReactNode }) {
  return (
    <div className="bg-void-800/60 p-4">
      <dt className="text-[10px] uppercase tracking-widest text-cyan/60">{term}</dt>
      <dd className="text-mute-200 mt-1 break-all text-sm">{value}</dd>
    </div>
  );
}

function DeployHeatmap({ days }: { days: { date: string; count: number }[] }) {
